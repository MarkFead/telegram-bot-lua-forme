...
if not doesDirectoryExist(u8(getWorkingDirectory()..'\\Telegram Control')) then
   if not doesDirectoryExist(getWorkingDirectory()..'\\Telegram Control\\logo.png') and not doesDirectoryExist(getWorkingDirectory()..'\\Telegram Control\\EagleSans-Regular.ttf') then
      function main()
	   	if not isSampfuncsLoaded() or not isSampLoaded() then return end
	   	while not isSampAvailable() do wait(100) end
	   	sampAddChatMessage('[Telegram Control]{FFFFFF} Вы не установили файлы для корректной работы скрипта! Установить их можно в BlastHack.', 0x308ad9)
	   	thisScript():unload()
	   end
	   return
   end
...
local spawnCmd = new.bool(jsonConfig['settings'].spawnCmd)
...
imgui.OnFrame(function() return WinState[0] end,
   function(player)
      ...
      imgui.BeginGroup()
         ...
         if imgui.AnimButton(u8'Автор', imgui.ImVec2(200,40), 30) then tab = 4 end
      imgui.EndGroup()
      imgui.SameLine()
      ...
      elseif tab == 4 then
         ...
         imgui.SetCursorPos(imgui.ImVec2((imgui.GetWindowWidth() * 1.5 - 700) / 2, (imgui.GetWindowHeight() - 250) / 2))
			imgui.BeginChild('Other', imgui.ImVec2(300, 195), true)
				...
				if imgui.AnimButton(u8('Blasthack аккаунт'), imgui.ImVec2(-1, 77.5), 30) then os.execute('explorer https://www.blast.hk/members/465668/') end
				if imgui.AnimButton(u8('Тема на BlastHack'), imgui.ImVec2(-1, 77.5), 30) then os.execute('explorer https://www.blast.hk/threads/190315/') end
			imgui.EndChild()
      ...
      elseif tab == 3 then
         ...
         imgui.BeginChild('settings', imgui.ImVec2(365, 419), false)
            ...
            imgui.BeginChild('settingsUnder', imgui.ImVec2(-1, -1), false)
               ...
               if imgui.Checkbox(u8' Выбирать меню в диалогах', spawnCmd) then 
                  jsonConfig['settings'].spawnCmd = spawnCmd[0]
                  json('Config.json'):save(jsonConfig)
               end
               imgui.Hint('spawnCmd', u8'Отправляет запрос в диалог в игре.\nКоманда в Telegram: /dialog [id]')
            ...
            if res_table then
               if res_table.update_id ~= updateid then
                  ...
                  if message_from_user then
                     ...
                     elseif textTg:match('^/dialog') then
                        if jsonConfig['settings'].spawnCmd then
                           sendTelegramNotification('Вы не ввели текст для отправки!')
                        elseif not jsonConfig['settings'].spawnCmd then
                           sendTelegramNotification('Данная функция отключена!\nВключить можно в настройках скрипта.')
                        end
                     elseif textTg2:match('^/dialog (.+)') then
                        if jsonConfig['settings'].spawnCmd then
                           function onShowDialog(id, style, title, button1, button2, text)
                              if textTg2:match('^/dialog (.+)') then
                              local sendid = textTg2:match('^/dialog (.+)')
                              if id == 917 and acvite then
                                    sampSendDialogResponse(id, 1, sendid, nill)
                                    sendTelegramNotification('Вы выбрали: "'..sendid..'" пункт')
                              elseif id == 758 and active then
                                    sampSendDialogResponse(id, 1, sendid, nill)
                                    sendTelegramNotification('Вы выбрали: "'..sendArg..'" пункт')
                                 end
                              end
                         end
